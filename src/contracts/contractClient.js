 ...
 * contractClient.js
 *
 * ALL reward logic goes through the deployed Soroban contracts on Stellar Testnet.
 * No localStorage. No local fallbacks. No Horizon payment bypass.
 * No mock data.
 *
 * Key fix: contractSendReward now reads the reward amount directly from the
 * transaction return value instead of making a second simulate() call after
 * the invoke. This prevents the bug where a post-invoke simulate failure
 * would return {success:false} even though XLM had already been transferred.
 */
...
/** Load admin keypair — throws clearly if env var is missing or invalid. */
function getAdminKeypair() {
  const secret = import.meta.env.VITE_ADMIN_SECRET?.trim();
  if (!secret || !secret.startsWith("S")) {
    throw new Error("VITE_ADMIN_SECRET is not set or invalid in .env");
...
/**
 * Simulate a read-only contract call and return the native JS value.
 * Throws on simulation error — no fallback.
 */
async function simulate(method, params = []) {
  const keypair = getAdminKeypair();
  const account = await rpcServer.getAccount(keypair.publicKey());
 ...
 * Returns { hash, returnValue } on success — returnValue is the native JS
 * value decoded from the transaction result so callers do NOT need a
 * second simulate() call to read what the contract returned.
 *
 * Throws on any error — no silent swallowing.
 */
async function invoke(method, params = []) {
  ...
  // Decode the return value directly from the confirmed transaction result.
  // This avoids a second simulate() call and prevents the race-condition bug
  // where a post-invoke simulate failure would report {success:false} even
  // after XLM had already been transferred on-chain.
...
/**
 * CONTRACT CALL: send_reward  (state-changing)
 *
 * Invokes send_reward() on the Soroban contract which:
 *   1. Calls calculate_reward() internally
 *   2. Transfers XLM from the contract to the player
 *   3. Calls QuizLeaderboard::record_session() via inter-contract call
 *
 * The reward amount is read from the transaction return value — no second
 * simulate needed, no risk of reporting failure after a successful transfer.
 *
 * Rust: pub fn send_reward(env, admin, player, correct_answers, total_questions, max_streak) -> i128
 */
export const contractSendReward = async (
  ...
  if (correctAnswers === 0) {
    return { success: true, totalReward: 0, baseReward: 0, streakBonus: 0, noReward: true };
  }
  ...
  const { hash, returnValue } = await invoke("send_reward", [
    adminAddress.toScVal(),
    playerAddress.toScVal(),
    nativeToScVal(correctAnswers, { type: "u32" }),
    nativeToScVal(totalQuestions, { type: "u32" }),
  ...
  // returnValue is the i128 total_reward returned by the Rust function
  const totalRewardStroops = returnValue !== null ? Number(returnValue) : 0;
  const totalReward        = totalRewardStroops / STROOPS_PER_XLM;

  // Derive base/streak split from the on-chain constants to show breakdown
  const baseReward  = (correctAnswers * 5_000_000) / STROOPS_PER_XLM;
  const streakBonus = totalReward - baseReward;
 ...
 *
 * Rust: pub fn calculate_reward(env, correct_answers: u32, max_streak: u32) -> RewardResult
 * Used only for display purposes (e.g. showing max reward on start screen).
 * Never used as a payment path.
 */
export const contractCalculateReward = async (correctAnswers, maxStreak) => {
  const result = await simulate("calculate_reward", [
    nativeToScVal(correctAnswers, { type: "u32" }),
  ...
  return {
    baseReward:  Number(result.base_reward)  / STROOPS_PER_XLM,
    streakBonus: Number(result.streak_bonus) / STROOPS_PER_XLM,
    totalReward: Number(result.total_reward) / STROOPS_PER_XLM,
  };
};
...
export const contractGetScore = async (playerPublicKey) => {
  ...
  return {
    correct: Number(result.correct),
    total:   Number(result.total),
    earned:  Number(result.earned) / STROOPS_PER_XLM,
  };
};
...
export const contractGetBalance = async () => {
  const result = await simulate("get_balance", []);
  return Number(result) / STROOPS_PER_XLM;
};

/**
 * CONTRACT CALL: initialize  (one-time setup, state-changing)
 *
 * Rust: pub fn initialize(env, admin, token, leaderboard)
 * Run once after deployment from the browser console.
 */
export const initializeContract = async (tokenAddress, leaderboardAddress) => {
  const keypair      = getAdminKeypair();
  const adminAddress = new Address(keypair.publicKey());

  const { hash } = await invoke("initialize", [
    adminAddress.toScVal(),
    new Address(tokenAddress).toScVal(),
    new Address(leaderboardAddress).toScVal(),
  ]);
